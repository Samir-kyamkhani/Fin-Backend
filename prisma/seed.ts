...
async function main() {
  ...
  // State Head IP whitelists
  await prisma.ipWhitelist.upsert({
    where: { domainName: 'http://localhost:5182' },
    update: {},
    create: {
      domainName: 'http://localhost:5182',
      serverIp: '127.0.0.1',
      rootId: null,
      userId: stateHeadUser1.id,
      createdAt: new Date(),
  ...
  await prisma.ipWhitelist.upsert({
    where: { domainName: 'http://localhost:5183' },
    update: {},
    create: {
      domainName: 'http://localhost:5183',
      serverIp: '127.0.0.1',
      rootId: null,
      userId: stateHeadUser2.id,
      createdAt: new Date(),
  ...
  // Master Distributor IP whitelists (ports 5176-5179)
  const masterDistributorPorts = [
    { user: masterDistributorUser1, port: 5176 },
    { user: masterDistributorUser2, port: 5177 },
    { user: masterDistributorUserA, port: 5178 },
    { user: masterDistributorUserX, port: 5179 },
  ];

  for (const { user, port } of masterDistributorPorts) {
    await prisma.ipWhitelist.upsert({
      where: { domainName: `http://localhost:${port}` },
      update: {},
      create: {
  ...
  // Distributor IP whitelists (ports 5180-5184)
  const distributorPorts = [
    { user: distributorUser1, port: 5180 },
    { user: distributorUser2, port: 5181 },
    { user: distributorUserA1, port: 5182 },
    { user: distributorUserB, port: 5183 },
    { user: distributorUserY, port: 5184 },
  ];

  for (const { user, port } of distributorPorts) {
    await prisma.ipWhitelist.upsert({
      where: { domainName: `http://localhost:${port}` },
      update: {},
      create: {
  ...
  // Retailer IP whitelists (ports 5185-5190)
  const retailerPorts = [
    { user: retailerUser1, port: 5185 },
    { user: retailerUser2, port: 5186 },
    { user: retailerUserA1, port: 5187 },
    { user: retailerUserB1, port: 5188 },
    { user: retailerUserC, port: 5189 },
    { user: retailerUserZ, port: 5190 },
  ];

  for (const { user, port } of retailerPorts) {
    await prisma.ipWhitelist.upsert({
      where: { domainName: `http://localhost:${port}` },
      update: {},
      create: {
  ...
  console.log('✅ All IP whitelists created');
  ...
  for (const { user, name } of usersToVerify) {
    ...
    if (verifyUser) {
      ...
      console.log(`- Hierarchy Level: ${verifyUser.role.hierarchyLevel}`);
      ...
      if (verifyUser.parent) {
        ...
        const parentLevel = verifyUser.parent.role?.hierarchyLevel || 0;
        const childLevel = verifyUser.role.hierarchyLevel;
  ...
  console.log('- State Head1: http://localhost:5182');
  console.log('- State Head2: http://localhost:5183');
  console.log('- Master Distributors: http://localhost:5176-5179');
  console.log('- Distributors: http://localhost:5180-5184');
  console.log('- Retailers: http://localhost:5185-5190');
  console.log('====================================\n');
}
